...
function resolvePlan(parsed, positionals, runtime) {
  ...
  const scope = parsed.scope || harness.defaultScope;
  ...
  const projectDirDefaulted = scope === 'project' && !parsed.projectDir;
  const projectDir = scope === 'project'
    ? resolvePath(parsed.projectDir || runtime.cwd, runtime.cwd, runtime.homeDir)
    : null;
...
function helpText() {
  return [
    ...
    '',
    'Install Polyglot eval-service agent skills into supported harness skill directories.',
    '',
    'Usage:',
    ...
    '  --project-dir <dir>    Project directory for project scope. Defaults to cwd when scope is project.',
    ...
    '  openclaw     workspace $(openclaw skills list --json).workspaceDir/skills/polyglot-service-client',
